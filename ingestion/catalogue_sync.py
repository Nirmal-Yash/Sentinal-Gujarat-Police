#!/usr/bin/env python3
"""Catalogue Sync — registry ingestion with source provenance preservation."""
...
def _coordinates(cam: dict):
    ...
    if not (-90 <= lat <= 90 and -180 <= lng <= 180) or (lat == 0 and lng == 0):
        return None, None, "unknown", None
    try: confidence = float(cam.get("coord_confidence", 0.5))
...
def _department(name: str, location: str):
    ...
    for needle, department in rules:
        if needle in value: return department, 0.45
...
def fetch_catalogue(retries: int = 5):
    ...
    for attempt in range(retries):
        try:
            response = requests.get(INGEST_API, timeout=10); response.raise_for_status(); data = response.json()
            if isinstance(data, list): return data
            if isinstance(data, dict): return data.get("cameras") or data.get("streams") or []
        except Exception as exc:
            log.warning("Catalogue fetch attempt %s/%s failed: %s", attempt + 1, retries, exc); time.sleep(delay); delay = min(delay * 2, 30)
    return []


def sync(conn=None) -> int:
    cameras = fetch_catalogue()
    if not cameras: log.warning("Empty catalogue — skipping sync"); return 0
    close_after = conn is None
    conn = conn or psycopg2.connect(DB_URL)
    rows=[]; coordinate_counts={}
    for cam in cameras:
        sid=int(cam.get("id",0)); urls=_build_urls(cam); name=cam.get("location") or cam.get("name") or f"Camera {sid}"; lat,lng,coord_source,coord_confidence=_coordinates(cam); department,department_confidence=_department(name,cam.get("location",""))
        provided_codec=str(cam["codec"]) if cam.get("codec") else None
        provided_width=int(cam["width"]) if cam.get("width") else None
        provided_height=int(cam["height"]) if cam.get("height") else None
        provided_fps=float(cam["fps"]) if cam.get("fps") else None
        if lat is not None: coordinate_counts[(lat,lng)] = coordinate_counts.get((lat,lng),0)+1
        rows.append((sid,name,cam.get("location",""),lat,lng,coord_source,coord_confidence,department,department_confidence,urls["rtsp"],urls["hls"],urls["whep"],provided_codec,provided_width,provided_height,provided_fps,provided_codec,provided_width,provided_height,provided_fps,"active" if cam.get("live",True) else "offline"))
    for coordinates,count in coordinate_counts.items():
        if count>1: log.warning("Catalogue has %s cameras at same coordinates %s",count,coordinates)
    sql="""
        INSERT INTO cameras
          (stream_id,name,location,lat,lng,coord_source,coord_confidence,department,department_confidence,rtsp_url,hls_url,whep_url,
           codec,width,height,fps,provided_codec,provided_width,provided_height,provided_fps,status,last_seen_at)
        VALUES %s
        ON CONFLICT (stream_id) DO UPDATE SET
          ...
          provided_codec=EXCLUDED.provided_codec,provided_width=EXCLUDED.provided_width,provided_height=EXCLUDED.provided_height,provided_fps=EXCLUDED.provided_fps,
          codec=COALESCE(EXCLUDED.codec,cameras.codec),width=COALESCE(EXCLUDED.width,cameras.width),height=COALESCE(EXCLUDED.height,cameras.height),fps=COALESCE(EXCLUDED.fps,cameras.fps),
          status=EXCLUDED.status,last_seen_at=NOW()
    """
    template="(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,NOW())"
    with conn.cursor() as cur: execute_values(cur,sql,rows,template=template)
    conn.commit()
    if close_after: conn.close()
    log.info("Catalogue sync: %s cameras upserted from %s",len(rows),INGEST_API); return len(rows)
...
def run_loop():
    ...
    for _ in range(20):
        ...
        except Exception as exc: log.info("Waiting for DB: %s",exc); time.sleep(3)
    while True:
        try: sync()
        except Exception as exc: log.error("Sync error: %s",exc,exc_info=True)
