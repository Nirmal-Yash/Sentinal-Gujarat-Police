...
The retired live.corp8.cloud ingest API is not used. The current provider is a
password-only web gateway which establishes a session cookie; ingestion keeps
that credential server-side and writes canonical camera stream endpoints into
the registry.
"""
import os
import time
...
def _coordinates(cam: dict):
    ...
    return (
        lat,
        lng,
        str(cam.get("coord_source") or "government_catalogue"),
...
def _department(name: str, location: str):
    ...
    rules = (
        ("ahmedabad", "AMC / Gujarat Police"),
        ("junagadh", "Gujarat Police Junagadh"),
        ("rajkot", "Rajkot Police"),
        ("navsari", "Navsari Police"),
        ("gandhinagar", "Gujarat Police Gandhinagar"),
        ("patan", "Gujarat Police Patan"),
...
def fetch_catalogue(retries: int = 5):
    ...
    for attempt in range(retries):
        try:
            ...
            log.info(
                "CCTV catalogue authenticated successfully: %s camera records",
                len(cameras),
            )
            return cameras
        except Exception as exc:
            log.warning(
                "CCTV catalogue fetch attempt %s/%s failed: %s",
                attempt + 1,
                retries,
                exc,
            )
...
def sync(conn=None) -> int:
    ...
    for cam in cameras:
        ...
        if sid in seen_stream_ids:
            raise RuntimeError(
                f"Duplicate stream_id in CCTV catalogue: {sid}"
            )
        ...
        department, department_confidence = _department(
            name,
            cam.get("location", ""),
        )
        ...
        provided_fps = (
            float(cam["fps"])
            if cam.get("fps") is not None
            else None
        )

        if lat is not None:
            coordinate_counts[(lat, lng)] = (
                coordinate_counts.get((lat, lng), 0) + 1
            )

        # IMPORTANT: last_seen_at is populated by NOW() in SQL, therefore it
        # must NOT have a corresponding tuple value.
        rows.append(
            (
                sid,
                name,
    ...
    for coordinates, count in coordinate_counts.items():
        if count > 1:
            log.warning(
                "CCTV catalogue has %s cameras at same coordinates %s",
                count,
                coordinates,
            )
    ...
    sql = """
        ...
        ON CONFLICT (stream_id) DO UPDATE SET
          ...
          lat=CASE
            WHEN EXCLUDED.lat IS NOT NULL
             AND COALESCE(cameras.coord_confidence,-1)
                 <= COALESCE(EXCLUDED.coord_confidence,-1)
            THEN EXCLUDED.lat
            ELSE cameras.lat
          END,
          lng=CASE
            WHEN EXCLUDED.lng IS NOT NULL
             AND COALESCE(cameras.coord_confidence,-1)
                 <= COALESCE(EXCLUDED.coord_confidence,-1)
            THEN EXCLUDED.lng
            ELSE cameras.lng
          ...
          coord_source=CASE
            WHEN EXCLUDED.lat IS NOT NULL
             AND COALESCE(cameras.coord_confidence,-1)
                 <= COALESCE(EXCLUDED.coord_confidence,-1)
            THEN EXCLUDED.coord_source
            ELSE cameras.coord_source
          END,
          coord_confidence=CASE
            WHEN EXCLUDED.lat IS NOT NULL
             AND COALESCE(cameras.coord_confidence,-1)
                 <= COALESCE(EXCLUDED.coord_confidence,-1)
            THEN EXCLUDED.coord_confidence
            ELSE cameras.coord_confidence
          END,
          department=CASE
            WHEN cameras.department='Unassigned'
              OR (
                cameras.department_source NOT IN ('manual','government_seed','import')
                AND COALESCE(cameras.department_confidence,-1)
                    <= COALESCE(EXCLUDED.department_confidence,-1)
              )
            THEN COALESCE(EXCLUDED.department,cameras.department)
            ELSE cameras.department
          END,
          department_source=CASE
            ...
            THEN CASE
              WHEN EXCLUDED.department IS NOT NULL
              THEN 'catalogue_inferred'
              ELSE cameras.department_source
            END
            ELSE cameras.department_source
          END,
          department_confidence=CASE
            WHEN cameras.department='Unassigned'
              OR (
                cameras.department_source NOT IN ('manual','government_seed','import')
                AND COALESCE(cameras.department_confidence,-1)
            ...
            THEN COALESCE(
              EXCLUDED.department_confidence,
              cameras.department_confidence
            )
            ELSE cameras.department_confidence
          END,
          ...
          provided_codec=EXCLUDED.provided_codec,
          provided_width=EXCLUDED.provided_width,
          provided_height=EXCLUDED.provided_height,
          provided_fps=EXCLUDED.provided_fps,
    ...
    # 23 placeholders: 21 source values + source_system + external_id.
    # last_seen_at is generated by NOW().
    template = (
    ...
    if rows and len(rows[0]) != expected_values:
        raise RuntimeError(
            "Catalogue row/template mismatch: "
            f"row has {len(rows[0])} values; expected {expected_values}"
        )
    ...
    log.info(
        "CCTV catalogue sync: %s cameras upserted",
        len(rows),
    )
...
def run_loop():
    ...
    while True:
        ...
        time.sleep(
            max(
                30,
                int(os.getenv("CATALOGUE_SYNC_INTERVAL", "300")),
            )
        )
