...
The previous live.corp8.cloud ingest API is retired. The current provider is a
...
def _build_urls(cam: dict) -> dict:
    ...
    # RTSP has been directly verified on the current infrastructure.
    rtsp = cam.get("rtsp_url") or f"rtsp://{RTSP_HOST_IP}:{RTSP_PORT}/stream/{canonical}"
    # Browser HLS is exposed by the API through the authenticated same-origin proxy.
    hls = f"/cctv/{canonical}/index.m3u8"
    whep = cam.get("whep_url") or f"http://{RTSP_HOST_IP}:8889/stream/{canonical}/whep"
