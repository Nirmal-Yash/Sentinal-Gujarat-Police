"""Persistence boundary for test mode. It has no production-table access."""
import json, os
...
def _timestamp(data):
    try: return datetime.fromisoformat(_value(data, "source_ts").replace("Z", "+00:00"))
...
def persist(data: dict):
    ...
    if not session_id or not detection_id: raise ValueError("test event is missing session_id or detection_id")
    timestamp, stream_id, kind = _timestamp(data), int(_value(data, "stream_id", "0") or 0), _value(data, "detection_type", "unknown")
    camera_label = f"Test Camera {stream_id}"; track_id = _value(data, "track_id") or None
    plate = "".join(value for value in _value(data, "plate_text").upper() if value.isalnum()) or None
    confidence = float(_value(data, "conf", "0") or 0); bbox = {key: _value(data, key) for key in ("x1","y1","x2","y2") if key.encode() in data}
