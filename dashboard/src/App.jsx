...
function useSpaRoute(){
 ...
 const navigate=useCallback((target,{replace=false}={})=>{const next=normalizePath(target),current=normalizePath(window.location.pathname);if(current!==next){window.history[replace?'replaceState':'pushState']({...window.history.state,sentinelRoute:next},'',next)}setPath(next)},[])
...
export default function App(){
 ...
 const ack=useCallback(async id=>transitionAlert({id},'ACKNOWLEDGED'),[transitionAlert])
