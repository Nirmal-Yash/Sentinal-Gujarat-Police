...
export default function App() {
  ...
  // Test mode deliberately replaces only the feed-grid/alert data source.
  // The map retains the canonical production registry and no production event
  // is written, acknowledged, or displayed as a test event.
  useEffect(() => {
  ...
  const openTest = useCallback(async () => {
    try {
      const session = await api.getActiveTestSession()
      if (session) return startTest(session)
    } catch (error) { console.warn('test session lookup:', error) }
    setShowTestDiagnostics(true)
  }, [startTest])
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', overflow: 'hidden' }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', gap: 14, fontSize: 11 }}>
          <span style={{ color: testMode ? 'var(--accent)' : 'var(--text2)' }}>
            {testMode ? 'ISOLATED TEST MODE' : `${cameras.length} cameras`}
          </span>
      ...
      <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
        <div style={{ flex: 1, overflow: 'hidden', position: 'relative', display: 'flex', flexDirection: 'column' }}>
          {mapExpanded ? (
            <MapView cameras={testMode ? productionCameras : cameras} alerts={testMode ? [] : alerts} focusCameraId={mapFocus.id} focusNonce={mapFocus.nonce} route={vehicleRoute.sightings} routeFocusNonce={vehicleRoute.nonce}/>
          ) : (
            <div style={{ flex: 1, minHeight: 0 }}>
            <CameraGrid cameras={cameras} alertsByCam={alertsByCam} analyticsByCam={analyticsByCam} pipelineStats={pipelineStats} onLocate={testMode ? (() => {}) : locateCamera} focusCameraId={cameraFocus.id} focusNonce={cameraFocus.nonce}/>
            </div>
          )}
        </div>
