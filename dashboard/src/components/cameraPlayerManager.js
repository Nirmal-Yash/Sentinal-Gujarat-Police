...
function ensureObserver(){
  ...
  observer=new IntersectionObserver(records=>{
    for(const record of records){
      ...
      if(item.visible){
        clearTimeout(item.suspendTimer); item.suspendTimer=null
        scheduleBudget()
...
function recompute(stagger){
  ...
  const eligible=[...entries.values()].filter(item=>item.visible&&!item.pageSuspended)
    .sort((a,b)=>distanceFromViewportCenter(a)-distanceFromViewportCenter(b))
  const limit=Math.min(MAX_PLAYERS,memoryLimit)
  const wanted=new Set(eligible.slice(0,limit).map(item=>item.id))
  ...
  entries.forEach(item=>{if(!item.visible) item.setActive(false)})
}
...
function onVisibilityChange(){
  ...
  if(pageHidden){
    ...
    entries.forEach(item=>{if(item.pageSuspended)item.pageSuspended=false})
    recompute(true)
  }
}
...
export function useCameraPlayerSlot(id,elementRef){
  ...
  useEffect(()=>{
    ...
    if(!observer){setActive(true);return undefined}
    const key=String(id)
    const item={id:key,element,setActive,active:false,visible:false,pageSuspended:false,suspended:false,suspendTimer:null,activationTimer:null}
    entries.set(key,item); elementToId.set(element,key); observer?.observe(element); scheduleBudget()
    return()=>{clearTimeout(item.suspendTimer);clearTimeout(item.activationTimer);observer?.unobserve(element);entries.delete(key);setActive(false)}
  },[id,elementRef])
  useEffect(()=>{const item=entries.get(String(id));if(item)item.active=active},[id,active])
  return active
}
