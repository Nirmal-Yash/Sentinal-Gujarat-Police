...
export default function CameraGrid({cameras=[],alertsByCam={},onLocate,focusCameraId,focusNonce=0,pipelineStats}){
  ...
  return <div style={{display:'flex',flexDirection:'column',width:'100%',maxWidth:'100%',minWidth:0,height:'100%',background:'var(--bg)',overflow:'hidden',boxSizing:'border-box'}}>
    ...
    <main style={{flex:1,minWidth:0,minHeight:0,width:'100%',maxWidth:'100%',overflowY:'auto',overflowX:'hidden',padding:10,boxSizing:'border-box'}}><div style={{display:'grid',width:'100%',minWidth:0,maxWidth:'100%',gridTemplateColumns:,gridTemplateColumns:`repeat(${cols},minmax(0,1fr))`,gap:8,boxSizing:'border-box'}}>{cameras.map(cam=><CameraCard key={cam.id} cam={cam} alertCount={alertsByCam[cam.id]||0} onFocus={open} onLocate={onLocate}/>)}</div></main>
