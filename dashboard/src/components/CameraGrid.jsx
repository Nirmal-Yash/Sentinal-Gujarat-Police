...
export default function CameraGrid({cameras=[],alertsByCam={},onLocate,focusCameraId,focusNonce=0,pipelineStats}){
  ...
  return <div style={{display:'flex',flexDirection:'column',height:'100%',background:'var(--bg)',overflow:'hidden'}}>
    <header style={{display:'flex',alignItems:'center',gap:10,padding:'8px 12px',borderBottom:'1px solid var(--border)',background:'var(--surface)'}}><b style={{fontSize:12}}>{cameras.length} Cameras</b>{pipelineStats&&<span style={{color:'var(--text2)',fontSize:10}}>Frames {Number(pipelineStats.raw_frames||0).toLocaleString()} · Detections {Number(pipelineStats.detections||0).toLocaleString()}</span>}<div style={{marginLeft:'auto',display:'flex',gap:3}}>{[2,3,4,5].map(n=><button key={n} onClick={()=>{setCols(n);localStorage.setItem('sentinel.camera-grid.columns.v1',String(n))}} aria-label={`${n} camera columns`} style={{width:28,height:26,borderRadius:5,border:`1px solid ${cols===n?'var(--accent)':'var(--border)'}`,background:cols===n?'var(--accent-soft)':'transparent',color:cols===n?'var(--accent)':'var(--text2)',cursor:'pointer'}}>{n}</button>)}</div></header>
    <main style={{flex:1,overflowY:'auto',padding:10}}><div style={{display:'grid',gridTemplateColumns:`repeat(${cols},minmax(0,1fr))`,gap:8}}>{cameras.map(cam=><CameraCard key={cam.id} cam={cam} alertCount={alertsByCam[cam.id]||0} onFocus={open} onLocate={onLocate}/>)}</div></main>
    {focused&&<div onClick={e=>e.target===e.currentTarget&&setFocused(null)} style={{position:'fixed',inset:0,zIndex:2000,display:'grid',placeItems:'center',padding:16,background:'rgba(0,0,0,.88)'}}><section style={{width:'min(92vw,1200px)',background:'var(--surface)',border:'1px solid var(--border)',borderRadius:10,overflow:'hidden'}}><header style={{padding:'10px 14px',display:'flex',gap:10,alignItems:'center',borderBottom:'1px solid var(--border)'}}><b>CAM-{paddedId(focused)}</b><span style={{color:'var(--text2)'}}>{focused.name}</span><button onClick={()=>setFocused(null)} style={{marginLeft:'auto',cursor:'pointer'}}>Close</button></header><div style={{position:'relative',width:'100%',aspectRatio:streamAspect(focused),background:'#000'}}><LivePlayer cam={focused} muted={false} active managed={false} fit="contain"/></div></section></div>}
  </div>
}
