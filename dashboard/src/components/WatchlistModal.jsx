...
export default function WatchlistModal({onClose,testMode=false,testSession=null}){
 ...
 return <div style={overlay} onClick={e=>e.target===e.currentTarget&&onClose()}><div style={modal}>
  ...
  <div style={{display:'flex',padding:'8px 14px',gap:8,borderBottom:'1px solid var(--border)',background:'var(--surface2)'}}>{[['list','View All'],['add','Add Entry']].map(([t,l])=><button key={t} type="button" onClick={()=>{setTab(t);setError('')}} style={{flex:1,padding:'8px 0',borderRadius:6,border:`1px solid ${tab===t?'var(--accent)':'transparent'}`,background:tab===t?'var(--accent)':'transparent',color:tab===t?'var(--on-accent)':'var(--text2)',cursor:'pointer',fontSize:13,fontWeight:tab===t?800:600,transition:'all .15s ease'}}>{l}</button>)}</div>
  <div style={{flex:1,overflowY:'auto',padding:18}}>
    {tab==='list'&&<><div style={{fontSize:13,color:'var(--text2)',marginBottom:10}}>{entries.length} active entries</div>{entries.map(e=><div key={e.id} style={{padding:'10px 12px',marginBottom:8,borderRadius:8,background:'var(--surface2)',border:'1px solid var(--border)',display:'flex',gap:10,alignItems:'flex-start'}}><div style={{fontSize:20}}>{e.entity_type==='vehicle'?'V':'P'}</div><div style={{flex:1,minWidth:0}}><div style={{fontWeight:600,fontSize:13}}>{e.name}</div>{e.plate_number&&<div style={{fontSize:12,color:'var(--accent)',marginTop:2}}>Plate: {e.plate_number}</div>}{e.entity_type==='person'&&<div style={{fontSize:10,color:'var(--text2)',marginTop:3}}>Face embedding: {e.is_active?'active':'inactive'}</div>}{e.description&&<div style={{fontSize:12,color:'var(--text2)',marginTop:2}}>{e.description}</div>}</div><div style={{display:'flex',flexDirection:'column',alignItems:'flex-end',gap:4}}><span style={{fontSize:10,fontWeight:800,color:e.alert_priority==='CRITICAL'||e.alert_priority==='HIGH'?'var(--high)':e.alert_priority==='MEDIUM'?'var(--medium)':'var(--low)',background:e.alert_priority==='CRITICAL'||e.alert_priority==='HIGH'?'rgba(255,59,48,.12)':e.alert_priority==='MEDIUM'?'rgba(255,179,0,.12)':'rgba(52,199,89,.12)',border:`1px solid ${e.alert_priority==='CRITICAL'||e.alert_priority==='HIGH'?'rgba(255,59,48,.35)':e.alert_priority==='MEDIUM'?'rgba(255,179,0,.35)':'rgba(52,199,89,.35)'}`,padding:'2px 7px',borderRadius:4}}>{e.alert_priority}</span><button onClick={()=>remove(e.id)} style={{fontSize:10,fontWeight:750,padding:'3px 8px',borderRadius:5,border:'1px solid var(--border-strong)',background:'var(--surface3)',color:'var(--red)',cursor:'pointer'}}>Remove</button></div></div>)}{entries.length===0&&<div style={{textAlign:'center',padding:32,color:'var(--text2)',fontSize:13}}>Watchlist empty</div>}</>}
    {tab==='add'&&<div>
     <label style={{fontSize:12,color:'var(--text2)',display:'block',marginBottom:4}}>Full Name *</label>{inp('name','Person / Vehicle name')}
     <label style={{fontSize:12,color:'var(--text2)',display:'block',marginBottom:6}}>Type</label><div style={{display:'flex',gap:8,marginBottom:10}}>{[['person','Person & Face'],['vehicle','Vehicle Plate']].map(([type,label])=><button key={type} type="button" onClick={()=>{change('entity_type',type);setPersonFile(null);setPersonCheck(null)}} style={{flex:1,padding:'8px 12px',borderRadius:6,border:`1px solid ${form.entity_type===type?'var(--accent)':'var(--border)'}`,background:form.entity_type===type?'var(--accent)':'var(--surface2)',color:form.entity_type===type?'var(--on-accent)':'var(--text2)',fontWeight:form.entity_type===type?800:500,fontSize:12,cursor:'pointer',transition:'all .15s ease'}}>{label}</button>)}</div>
     {form.entity_type==='person'?<div style={{padding:12,border:'1px dashed var(--accent-border)',borderRadius:8,background:'var(--accent-soft)',marginBottom:8}}><div style={{fontSize:11,color:'var(--text2)',marginBottom:8}}>Upload exactly one clear reference face. The image is validated before the embedding is stored.</div><input id="watchlist-person-photo" type="file" accept="image/*" onChange={choosePerson} style={{display:'none'}}/><label htmlFor="watchlist-person-photo" style={{display:'inline-flex',alignItems:'center',gap:6,padding:'8px 12px',borderRadius:7,background:'var(--accent)',color:'var(--on-accent)',border:'1px solid var(--accent)',fontWeight:800,fontSize:11,cursor:'pointer'}}>{personFile?'Replace photo':'Upload photo'}</label>{checking&&<span style={{marginLeft:8,fontSize:10,color:'var(--text2)'}}>Checking…</span>}{personCheck&&<span style={{marginLeft:8,fontSize:10,color:personCheck.valid&&Number(personCheck.face_count)===1?'var(--green)':'var(--red)',fontWeight:700}}>{personCheck.valid?`Face detected · ${personCheck.face_count}`:'No visible face'}</span>}</div>:inp('plate_number','License plate (e.g. GJ03AA1234)')}
     {inp('description','Description / notes')}<label style={{fontSize:12,color:'var(--text2)',display:'block',marginBottom:4}}>Alert Priority</label><select value={form.alert_priority} onChange={e=>change('alert_priority',e.target.value)} style={{width:'100%',padding:'8px 10px',borderRadius:6,marginBottom:12,border:'1px solid var(--border)',background:'var(--surface2)',color:'var(--text)',fontSize:13}}><option>CRITICAL</option><option>HIGH</option><option>MEDIUM</option><option>LOW</option></select>{error&&<div style={{padding:9,marginBottom:10,borderRadius:7,border:'1px solid rgba(248,113,113,.4)',background:'rgba(248,113,113,.08)',color:'var(--high)',fontSize:11}}>{error}</div>}<button onClick={submit} disabled={loading||checking} style={{width:'100%',padding:'10px',borderRadius:7,border:'none',background:'var(--accent)',color:'var(--on-accent)',fontWeight:800,fontSize:13,cursor:loading||checking?'default':'pointer',opacity:loading?.6:1,boxShadow:'0 4px 14px rgba(0,0,0,.2)'}}>{loading?'Adding…':form.entity_type==='person'?'Add Person to Watchlist':'Add to Watchlist'}</button>
    </div>}
