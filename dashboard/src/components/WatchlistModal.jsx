...
export default function WatchlistModal({onClose,testMode=false,testSession=null}){
 ...
 return <div style={overlay} onClick={e=>e.target===e.currentTarget&&onClose()}><div style={modal}>
  ...
  <div style={{flex:1,overflowY:'auto',padding:18}}>
    {tab==='list'&&<><div style={{fontSize:13,color:'var(--text2)',marginBottom:10}}>{entries.length} active entries</div>{entries.map(e=><div key={e.id} style={{padding:'10px 12px',marginBottom:8,borderRadius:8,background:'var(--surface2)',border:'1px solid var(--border)',display:'flex',gap:10,alignItems:'flex-start'}}><div style={{fontSize:20}}>{e.entity_type==='vehicle'?'V':'P'}</div><div style={{flex:1,minWidth:0}}><div style={{fontWeight:600,fontSize:13}}>{e.name}</div>{e.plate_number&&<div style={{fontSize:12,color:'var(--accent)',marginTop:2}}>Plate: {e.plate_number}</div>}{e.entity_type==='person'&&<div style={{fontSize:10,color:'var(--text2)',marginTop:3}}>Face embedding: {e.is_active?'active':'inactive'}</div>}{e.description&&<div style={{fontSize:12,color:'var(--text2)',marginTop:2}}>{e.description}</div>}</div><div style={{display:'flex',flexDirection:'column',alignItems:'flex-end',gap:4}}><span style={{fontSize:10,fontWeight:700,color:e.alert_priority==='CRITICAL'||e.alert_priority==='HIGH'?'var(--high)':'var(--medium)',background:(e.alert_priority==='CRITICAL'||e.alert_priority==='HIGH'?'var(--high)':'var(--medium)')+'22',padding:'1px 6px',borderRadius:4}}>{e.alert_priority}</span><button onClick={()=>remove(e.id)} style={{fontSize:11,padding:'2px 8px',borderRadius:4,border:'1px solid var(--border)',background:'transparent',color:'var(--red)',cursor:'pointer'}}>Remove</button></div></div>)}{entries.length===0&&<div style={{textAlign:'center',padding:32,color:'var(--text2)',fontSize:13}}>Watchlist empty</div>}</>}
    {tab==='add'&&<div>
