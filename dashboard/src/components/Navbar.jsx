...
export default function Navbar({ alertCount, onSearchOpen, onWatchlistOpen, onOnboardOpen, onVendorsOpen, onTestOpen, testMode, onReportExport, principal, onLogout, path='/', onNavigate }) {
  ...
  return (
    <nav style={{ display:'flex', alignItems:'center', gap:8, padding:'0 16px', height:52, flexShrink:0, background:'var(--surface)', borderBottom:'1px solid var(--border)', overflowX:'auto' }}>
      <ShieldIcon/>
      <div style={{ lineHeight:1.2, minWidth:215 }}><div style={{ fontSize:14, fontWeight:800, letterSpacing:.8, color:'var(--text)' }}>SENTINEL AI</div><div style={{ fontSize:9, color:'var(--text2)', letterSpacing:.6, textTransform:'uppercase' }}>Gujarat Police Innovation Challenge 2026</div></div>
      <div style={{ width:1, height:28, background:'var(--border)', marginLeft:2 }}/>
      <div style={{ display:'flex', alignItems:'center', gap:5 }}><span style={{ width:7,height:7,borderRadius:'50%',background:'var(--green)',boxShadow:'0 0 7px var(--green)' }}/><span style={{ fontSize:11,color:'var(--green)',fontWeight:700,letterSpacing:.5 }}>LIVE</span></div>
      <div style={{ fontSize:12,color:'var(--text2)',fontVariantNumeric:'tabular-nums',marginRight:4 }}>{timeStr} <span style={{ fontSize:11,opacity:.7 }}>{dateStr}</span></div>
      <div style={{ display:'flex', gap:5 }}>
        <a href="/dashboard" onClick={go('/dashboard')} style={routeButton(path === '/dashboard')}>Dashboard</a>
        <a href="/feeds" onClick={go('/feeds')} style={routeButton(path === '/feeds')}>Feeds</a>
        <a href="/map" onClick={go('/map')} style={routeButton(path === '/map')}>Map</a>
        <a href="/alerts" onClick={go('/alerts')} style={routeButton(path === '/alerts')}>Alerts</a>
        <a href="/investigations" onClick={go('/investigations')} style={routeButton(path === '/investigations')}>Investigate</a>
      </div>
      <div style={{ flex:1, minWidth:8 }}/>
      {alertCount > 0 && <div style={{ display:'flex',alignItems:'center',gap:6,background:'rgba(248,81,73,.12)',border:'1px solid rgba(248,81,73,.3)',borderRadius:6,padding:'4px 9px' }}><BellIcon/><span style={{ fontSize:12,fontWeight:700,color:'var(--high)',whiteSpace:'nowrap' }}>{alertCount} unacknowledged</span></div>}
      <button style={btn} onClick={onSearchOpen}><SearchIcon/> Search</button>
      <button style={btn} onClick={onWatchlistOpen}><ListIcon/> Watchlist</button>
      {principal && ['ADMIN','SUPERADMIN'].includes(principal.role) && <>
        ...
        <button style={btn} onClick={onReportExport}>Export</button>
      </>}
      ...
      {principal && <span style={{ padding:'4px 8px',borderRadius:5,background:'var(--surface2)',color:'var(--text2)',fontSize:10,whiteSpace:'nowrap' }}>{principal.username} · {principal.role}</span>}
