...
export default function SearchModal({ onClose, onViewCamera, onLocateCamera, onLocateRoute, init, testMode = false, testSession = null }) {
  ...
  useEffect(() => { if (init?.tab) setTab(init.tab); if (init?.query) setQuery(init.query) }, [init])
  useEffect(() => {
    ...
    const timer = setTimeout(async () => { setLoading(true); setError(''); try {
      ...
      if (testMode && testSession) {
        if (tab === 'camera') {
          const cameras = await api.getTestCameras(testSession.id)
          const needle = value.toLowerCase()
          response = { query: value, items: cameras.filter(camera => [camera.id, camera.stream_id, camera.name, camera.location, camera.status].some(field => String(field ?? '').toLowerCase().includes(needle))), limit: 20, offset: 0 }
        } else response = await api.getTestResults(testSession.id, { plate: value, detection_type: 'plate', limit: 100 })
      } else response = tab === 'camera' ? await api.searchCameras(value, { signal: controller.signal }) : await api.searchPlate(value, { signal: controller.signal })
      if (request === sequenceRef.current) setResults(response)
    } catch (err) { if (err.name !== 'AbortError' && request === sequenceRef.current) setError(err.message) } finally { if (request === sequenceRef.current) setLoading(false) } }, 250)
    return () => { clearTimeout(timer); controller.abort() }
  }, [query, tab, testMode, testSession?.id])
