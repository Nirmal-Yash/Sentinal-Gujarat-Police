...
export default function AlertTypeBadge({ type = 'alert', compact = false }) {
  ...
  return <span aria-label={`Alert type ${label}`} style={{ display: 'inline-flex', alignItems: 'center', minHeight: compact ? 20 : 22, padding: compact ? '0 6px' : '0 8px', borderRadius: 6, background: 'var(--accent-soft)', border: '1px solid var(--accent-border)', color: 'var(--accent-strong)', fontSize: compact ? 8 : 9, fontWeight: 800 }}>{label}</span>
}
