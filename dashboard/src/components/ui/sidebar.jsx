...
export function Sidebar({children}){const {open}=useSidebar();return <aside data-state={open?'expanded':'collapsed'} className="ui-sidebar">{children}</aside>}
...
export function SidebarTrigger({className=''}){
 ...
 return <button type="button" className={`ui-icon-button ${className}`.trim()} onClick={toggle} aria-label={open?'Collapse navigation sidebar':'Expand navigation sidebar'} title={open?'Collapse navigation':'Expand navigation'}>{open?<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="m15 18-6-6 6-6"/></svg>:<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="m9 18 6-6-6-6"/></svg>}</button>
}
