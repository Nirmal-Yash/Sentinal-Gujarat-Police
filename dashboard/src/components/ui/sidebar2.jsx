...
export function SidebarTrigger({className=''}){const{toggle,open}=useSidebar();return <button type="button" className={`ui-icon-button ${className}`.trim()} onClick={toggle} aria-label={open?'Collapse navigation sidebar':'Expand navigation sidebar'} title={open?'Collapse navigation':'Expand navigation'}>{open?<span aria-hidden="true">‹</span>:<span aria-hidden="true">›</span>}</button>}
