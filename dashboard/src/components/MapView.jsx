...
export default function MapView({ cameras, alerts = [], compact = false, focusCameraId, focusNonce = 0, route = [], routeFocusNonce = 0 }) {
  ...
  return <div style={{ position: 'relative', height: '100%', width: '100%' }}><div ref={containerRef} style={{ height: '100%', width: '100%', background: '#1a1f2e' }}/>{(notice || registryNotice) && <div style={{ position: 'absolute', top: 12, left: 52, maxWidth: 460, padding: '8px 10px', borderRadius: 6, background: 'rgba(18,24,34,.94)', border: '1px solid var(--border)', color: 'var(--text)', fontSize: 11, pointerEvents: 'none' }}>{notice || registryNotice}</div>}<div style={{ position: 'absolute', right: 12, top: 12, padding: '5px 8px', borderRadius: 5, background: 'rgba(18,24,34,.9)', color: 'var(--text2)', fontSize: 10, pointerEvents: 'none' }}>{mappedCount}/{cameras.length} mapped{reviewCount ? ` · ${reviewCount} need review` : ''}</div></div>
}
