...
async def session_state(session_id: uuid.UUID, _: Principal = Depends(require_role("ADMIN")), db: AsyncSession = Depends(get_db)):
    ...
    camera_rows = (await db.execute(text("""
        SELECT f.id,f.stream_id,f.camera_label,f.hls_path,f.width,f.height,f.fps,f.location,f.lat,f.lng,s.status
    ...
    cameras = [{
        ...
        "location": row["location"] or "Isolated video test", "lat": row["lat"], "lng": row["lng"],
...
async def session_cameras(session_id: uuid.UUID, _: Principal = Depends(require_role("ADMIN")), db: AsyncSession = Depends(get_db)):
    enabled(); rows = (await db.execute(text("SELECT f.id,f.stream_id,f.camera_label,f.hls_path,f.width,f.height,f.fps,f.location,f.lat,f.lng,s.status FROM test_session_feeds f JOIN test_sessions s ON s.id=f.session_id WHERE f.session_id=CAST(:id AS uuid) ORDER BY f.stream_id"), {"id": str(session_id)})).mappings().all()
    if not rows: raise HTTPException(404, "Test session not found")
    return [{"id": f"test-{row['id']}","stream_id": row["stream_id"],"name": row["camera_label"],"location": row["location"] or "Isolated video test", "lat": row["lat"], "lng": row["lng"],"hls_url": row["hls_path"],"whep_url": "","stream_url": f"/api/test/sessions/{session_id}/feeds/{row['stream_id']}/video","codec": "H.264 (MediaMTX)","width": row["width"],"height": row["height"],"fps": row["fps"],"effective_codec": "H.264 (MediaMTX)","effective_width": row["width"],"effective_height": row["height"],"effective_fps": row["fps"],"status": row["status"],"health_status": row["status"],"connectivity_status": row["status"],"is_test": True} for row in rows]
