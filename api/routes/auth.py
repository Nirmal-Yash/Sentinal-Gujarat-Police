...
async def login(body: Login, request: Request, response: Response, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        await db.commit()
    except HTTPException: await db.rollback(); raise
...
async def refresh(response: Response, refresh_token: str | None = Cookie(default=None, alias=REFRESH_COOKIE_NAME), db: AsyncSession = Depends(get_db)):
    ...
    except Exception as exc: raise HTTPException(401, 'Invalid or expired refresh token') from exc
    revoked = await db.scalar(text('SELECT 1 FROM revoked_tokens WHERE jti=CAST(:jti AS uuid) AND expires_at>NOW()'), {'jti': jti})
    if revoked:
        await db.execute(text('UPDATE user_sessions SET revoked=TRUE WHERE user_id=CAST(:uid AS uuid)'), {'uid': uid}); await db.commit(); raise HTTPException(401, 'Refresh token reuse detected')
    ...
    if not row or row['revoked']: raise HTTPException(401, 'Session is no longer valid')
...
async def logout(response: Response, principal: Principal = Depends(current_principal), db: AsyncSession = Depends(get_db)):
    if principal.jti: await db.execute(text('UPDATE user_sessions SET revoked=TRUE WHERE jti=CAST(:jti AS uuid)'), {'jti': principal.jti}); await db.commit()
...
async def create_user(body: UserCreate, principal: Principal = Depends(require_role('SUPERADMIN')), db: AsyncSession = Depends(get_db)):
    ...
    except Exception as exc: await db.rollback(); raise HTTPException(409, 'Username already exists') from exc
