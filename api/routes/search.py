...
import re, os, base64, json, time, uuid, asyncio, io
...
async def search_cameras(q: str = Query(..., min_length=1, max_length=100), limit: int = Query(20, ge=1, le=100), offset: int = Query(0, ge=0), db: AsyncSession = Depends(get_db)):
    result = await db.execute(text('''SELECT id, stream_id, name, location, lat, lng, hls_url, whep_url,
        ...
        ORDER BY similarity(name,:needle) DESC,stream_id LIMIT :limit OFFSET :offset'''),{'needle':q.strip(),'pattern':f'%{q.strip()}%','limit':limit,'offset':offset})
...
async def search_plate(q:str=Query(...,min_length=1,max_length=100),x_test_session_id:str|None=Header(None,alias='X-Test-Session-Id'),limit:int=Query(20,ge=1,le=100),db:AsyncSession=Depends(get_db)):
    ...
    if x_test_session_id:
        ...
        if not await db.scalar(text("SELECT 1 FROM test_sessions WHERE id=CAST(:id AS uuid) AND status IN ('starting','active')"),{'id':session_uuid}):raise HTTPException(404,'Test session not active')
...
async def _run_person_analysis(payload:bytes,timeout:float,operation:str):
    ...
    r=redis_async.from_url(os.getenv('REDIS_URL','redis://localhost:6379'),decode_responses=False);job_id=uuid.uuid4().hex;image_key=f'person:image:{job_id}';result_key=f'person:result:{job_id}'
    try:
        await r.set(image_key,payload,ex=max(30,int(timeout)+10));await r.xadd('person:investigations',{'request_id':job_id,'image_key':image_key,'result_key':result_key,'operation':operation},maxlen=1000,approximate=True)
