...
async def verify_audit_chain(_: Principal = Depends(require_permission("audit:read")), db: AsyncSession = Depends(get_db)):
    ...
    return {"valid": all(bool(row["valid"]) for row in rows), "entries": len(rows), "invalid_entries": [row["audit_seq"] for row in rows if not row["valid"]], "checked_at": __import__("datetime").datetime.now(__import__("datetime").timezone.utc).isoformat()}
