...
COOKIE_MAX_AGE = max(300, int(os.getenv("AUTH_COOKIE_MAX_AGE", str(REFRESH_TOKEN_HOURS * 3600))))
security = HTTPBearer(auto_error=False)
ROLE_ORDER = {"VIEWER": 1, "OPERATOR": 2, "INVESTIGATOR": 2, "AUDITOR": 2, "ADMIN": 3, "SUPERADMIN": 4}
ROLE_PERMISSIONS = {"VIEWER": {"camera:read", "alert:read", "search:read", "evidence:read"}, "OPERATOR": {"camera:read", "alert:read", "alert:operate", "search:read", "evidence:read", "evidence:create"}, "INVESTIGATOR": {"camera:read", "alert:read", "alert:operate", "search:read", "report:read", "evidence:read", "evidence:create"}, "AUDITOR": {"camera:read", "alert:read", "search:read", "report:read", "evidence:read", "audit:read"}, "ADMIN": {"camera:read", "camera:write", "alert:read", "alert:operate", "search:read", "report:read", "evidence:read", "evidence:create", "registry:admin", "audit:read"}, "SUPERADMIN": {"camera:read", "camera:write", "alert:read", "alert:operate", "search:read", "report:read", "evidence:read", "evidence:create", "registry:admin", "audit:read", "system:admin"}}
...
class Principal:
    ...
    role: str
    jti: str | None = None
def hash_password(value: str) -> str: return bcrypt.hashpw(value.encode(), bcrypt.gensalt()).decode()
def verify_password(value: str, stored: str) -> bool:
    try: return bcrypt.checkpw(value.encode(), stored.encode())
    except (ValueError, TypeError): return False
def _require_signing_secrets() -> None:
    if not SECRET_KEY or SECRET_KEY == "sentinel-change-in-production": raise HTTPException(503, "JWT signing secret is not configured")
    if not REFRESH_SECRET or REFRESH_SECRET == SECRET_KEY: raise HTTPException(503, "JWT refresh signing secret is not configured")
...
async def principal_from_token(token: str, db: AsyncSession) -> Principal:
    ...
    except (jwt.PyJWTError, KeyError) as exc: raise HTTPException(status.HTTP_401_UNAUTHORIZED, "Invalid or expired token") from exc
    row = (await db.execute(text("SELECT u.username, u.role FROM users u JOIN user_sessions s ON s.user_id=u.id WHERE u.id=CAST(:uid AS uuid) AND s.jti=CAST(:jti AS uuid) AND u.is_active=TRUE AND s.revoked=FALSE AND s.expires_at > NOW()"), {"uid": user_id, "jti": jti})).mappings().first()
...
def require_permission(permission: str):
    async def dependency(principal: Principal = Depends(current_principal)) -> Principal:
        if not has_permission(principal, permission): raise HTTPException(status.HTTP_403_FORBIDDEN, f"Permission required: {permission}")
        return principal
    return dependency
def require_role(minimum: str):
    async def dependency(principal: Principal = Depends(current_principal)) -> Principal:
        if ROLE_ORDER.get(principal.role, 0) < ROLE_ORDER[minimum]: raise HTTPException(status.HTTP_403_FORBIDDEN, "Insufficient role")
        return principal
    return dependency
