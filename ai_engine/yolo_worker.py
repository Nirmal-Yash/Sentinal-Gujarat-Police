...
IN_STREAM=f"{PREFIX}raw_frames"; RESET_STREAM=f"{PREFIX}cam_resets"; OUT_STREAM=f"{PREFIX}detections"
TRACK_HASH_PREFIX=f"{PREFIX}vehicle_tracks:"
OUT_MAX=5000; INFER_SIZE=int(os.getenv("INFER_SIZE","416"))
...
class WorkerState:
    def __init__(self): self.tracker=DeepSort(max_age=30,n_init=3); self.frame_ctr=0
    def reset(self): self.tracker=DeepSort(max_age=30,n_init=3); self.frame_ctr=0; log.info("Tracker reset")
...
def run():
    ...
    model=YOLO(YOLO_MODEL); r=redis.from_url(REDIS_URL,decode_responses=False)
    consumer=f"yolo-{uuid.uuid4().hex[:8]}"; _ensure_group(r,IN_STREAM,GROUP); _ensure_group(r,RESET_STREAM,"reset_watchers")
    states={}; last_reset_id=b"0"
    while True:
        try:
            ...
            for _,entries in resets or []:
                for reset_id,data in entries:
                    ...
                    if cam_id in states: states[cam_id].reset()
                    last_reset_id=reset_id
            msgs=r.xreadgroup(GROUP,consumer,{IN_STREAM:">"},count=4,block=200)
        except redis.exceptions.ResponseError as exc:
            if "NOGROUP" in str(exc): _ensure_group(r,IN_STREAM,GROUP); continue
            raise
        if not msgs: continue
        for _,entries in msgs:
            for msg_id,data in entries:
                try:
                    cam_id=data.get(b"cam_id",b"").decode(); pts_ms=int(data.get(b"pts_ms",b"0")); frame=_decode_frame(data)
                    if frame is None: continue
                    state=states.setdefault(cam_id,WorkerState()); state.frame_ctr+=1
                    if state.frame_ctr % FRAME_SKIP: continue
                    h,w=frame.shape[:2]; scale=INFER_SIZE/max(h,w)
                    ...
                    for box in results.boxes:
                        cls=int(box.cls[0])
                        if cls not in TARGET_CLS: continue
                        x1,y1,x2,y2=box.xyxy[0].tolist()
                        if scale<1: x1,y1,x2,y2=x1/scale,y1/scale,x2/scale,y2/scale
                        ds.append(([x1,y1,x2-x1,y2-y1],float(box.conf[0]),TARGET_CLS[cls]))
                    tracks=state.tracker.update_tracks(ds,frame=frame)
                    track_key=f"{TRACK_HASH_PREFIX}{cam_id}"
                    for track in tracks:
                        if not track.is_confirmed(): continue
                        l,t,r2,b=[int(v) for v in track.to_ltrb()]; etype=track.det_class or "person"
                        event=detection_event(data,etype,track_id=track.track_id,conf=track.det_conf or 0,x1=l,y1=t,x2=r2,y2=b,frame_w=w,frame_h=h)
                        r.xadd(OUT_STREAM,event,maxlen=OUT_MAX,approximate=True)
                        if etype in ("car","motorcycle","bus","truck"):
                            cx,cy=(l+r2)//2,(t+b)//2
                            value=f"{l}:{t}:{r2}:{b}:{cx}:{cy}:{pts_ms}:{float(track.det_conf or 0)}:{etype}"
                            r.hset(track_key,str(track.track_id),value)
                            r.hset(track_key,f"pts:{pts_ms}:{track.track_id}",value)
                            r.expire(track_key,10)
                except Exception: log.error("YOLO error",exc_info=True)
