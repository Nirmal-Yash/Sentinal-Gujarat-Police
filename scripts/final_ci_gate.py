...
required = [
    ...
    'intelligence/event_schema.py' if (ROOT/'intelligence/event_schema.py').exists() else 'ai_engine/event_schema.py',
    'intelligence/sighting_store.py','intelligence/alert_engine.py','intelligence/evidence_capture.py',
...
for p in required: print('[%s] %s' % ('OK' if p not in missing else 'FAIL', p))
files=sorted((ROOT/'database/migrations').glob('*.sql'))
versions=[re.match(r'^(\d+)_',p.name).group(1) for p in files if re.match(r'^(\d+)_',p.name)]
if len(versions)!=len(set(versions)): missing.append('duplicate migration versions')
if '012_sighting_deduplication.sql' in [p.name for p in files]: missing.append('duplicate 012 migration remains')
