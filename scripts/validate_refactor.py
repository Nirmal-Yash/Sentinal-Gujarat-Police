...
def main() -> int:
    ...
    conflict = ai.TrackANPRState(); conflict.add(obs("GJ01AB1234", .99, .95, .95, True, 1.0)); conflict.add(obs("GJ01AB1238", .99, .95, .95, True, 2.0))
    require(conflict.consensus(2)[0] is None, "conflicting high-confidence OCR is not merged")

    throttle = ai.TrackANPRState(); require(ai.should_run_ocr(throttle, 1.0, .8), "OCR initially dispatches")
    throttle.last_ocr_at = 1.0; require(not ai.should_run_ocr(throttle, 1.5, .8), "OCR interval suppresses excessive calls"); require(ai.should_run_ocr(throttle, 1.81, .8), "OCR resumes after configured interval")
    ...
    from api.routes.alerts import VALID_TRANSITIONS
    require(VALID_TRANSITIONS["NEW"] == {"ACKNOWLEDGED"}, "NEW alert has a safe acknowledgement transition")
    require(VALID_TRANSITIONS["ACKNOWLEDGED"] == {"INVESTIGATING", "RESOLVED"}, "acknowledged alert supports investigation/resolution")
    require(VALID_TRANSITIONS["INVESTIGATING"] == {"RESOLVED"}, "investigating alert resolves only after investigation")
    require(VALID_TRANSITIONS["RESOLVED"] == {"CLOSED"}, "resolved alert can be closed")
    require(VALID_TRANSITIONS["CLOSED"] == set(), "closed alert is terminal")
