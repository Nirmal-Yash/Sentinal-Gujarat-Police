#!/usr/bin/env python3
"""Seed a curated Test Mode demo session for evaluator walkthrough."""
import json
import os
import sys
...
USER = os.getenv("BOOTSTRAP_ADMIN_USERNAME", "admin")
PASSWORD = os.getenv("BOOTSTRAP_ADMIN_PASSWORD", "admin")
DEMO_SESSION_NAME = os.getenv("DEMO_SESSION_NAME", "Evaluator Demo Session")
ROUTE_CONFIG = Path(__file__).resolve().parents[1] / "config" / "test_vehicle_route.json"
...
def load_route_config():
    if not ROUTE_CONFIG.exists():
        return {"demo_plate": os.getenv("DEMO_PLATE", "GJ01AB1234"), "feeds": []}
...
def main():
    print("=== SEED DEMO TEST SESSION ===")
    token = login()
    route = load_route_config()
    demo_plate = route.get("demo_plate") or os.getenv("DEMO_PLATE", "GJ01AB1234")
    ...
    if not assets:
        print("No test video assets found. Upload videos via Test Mode first.")
        return 1
    ...
    session = api(
        ...
        token,
        {"name": DEMO_SESSION_NAME, "cameras": cameras},
    )
    ...
    watchlist = api(
        "POST",
        f"/api/v1/test/sessions/{session_id}/watchlist",
        token,
        {
    ...
    print(f"Seeded watchlist entry: {watchlist.get('id')} plate={demo_plate}")

    feeds = api("GET", f"/api/v1/test/sessions/{session_id}/cameras", token)
    print(f"Attached feeds: {len(feeds)}")
    for feed in feeds:
        print(f"  stream {feed.get('stream_id')}: {feed.get('name')} @ {feed.get('location')}")
